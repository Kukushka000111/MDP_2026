...
function AttendingCard({ item, onOpen }) {
  ...
  return (
    <article
      ...
      className="list-row-card flex cursor-pointer items-center gap-4 p-4 hover:border-indigo-200 dark:hover:border-indigo-700"
    >
      {item.image_url ? (
        <img
          src={item.image_url}
        ...
        <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-indigo-50 to-slate-100 text-lg font-bold text-slate-900 dark:from-indigo-950 dark:to-slate-800 dark:text-indigo-200">
          {(item.title || "?").charAt(0)}
        </div>
      )}
      <div className="min-w-0 flex-1">
        <h3 className="truncate font-bold text-slate-900 dark:text-slate-100">{item.title}</h3>
        <p className="mt-0.5 text-sm text-muted">{dateStr}</p>
        {item.registration_message && (
          <p className="mt-1 truncate text-xs text-muted">«{item.registration_message}»</p>
        )}
      </div>
      <span
  ...
  return (
    <section className="mb-8 space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-muted">Заявки на события и мероприятия, которые вы организуете</p>
      </div>
